...
if (post()){
	...
	if (data("item") == '+'){
		...
		$_DATA["avg"] = data('price');
		sql("INSERT INTO items SET ". fill_sql("itm", $item_cols) ."");
		$_DATA['item'] = sql_lastid();
	} elseif (data("count")){
		$res = sql("SELECT COUNT(sti_id) as pocet, itm_avg FROM storage_items LEFT JOIN items ON itm_id = sti_item WHERE sti_item = '". data('item') ."' AND sti_available = 'Y'");
		if ($row = sql_obj($res)) sql("UPDATE items SET itm_avg = '". ((($row->itm_avg * $row->pocet) + (data('price') * data('count'))) / (data('count') + $row->pocet)) ."' WHERE itm_id = '". data('item') ."'");
	}
...
print blok_start('Zboží');
	...
	print row_multi_start();
		print row_input('price', 'Nákupní cena bez DPH', 'Kč');
...
print blok_start('Naposledy přijaté zboží');
	include('./pages/prijem/porizeni/blok/naposledy_prijate_zbozi.php');
print blok_end();
//--------------------------------//
print column_end();
print columns_end();
